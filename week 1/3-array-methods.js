...
let fullNames = [];
 runners.forEach(generateFullnames);
/* for each works wih function and collect two argurement first on is the current position value like let say we have array of 2 element of beans and rice u know if we use for loop index 0 is beans and index 1 is rice so the first argument represend each value and he second argument represent there index is that fine :)*/
function generateFullnames(value,index){
    fullNames.push((value.first_name+" "+value.last_name).toString());
    return fullNames;
}
     

//am not sure if the to string is necessary sha
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = runners.map(function(val){ return val.first_name.toUpperCase().toString()});

  

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter(function(val){
    return val.shirt_size == 'L';
});

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let runnersDonations = runners.map(function(val){
    return val.donation;
});
let ticketPriceTotal = runnersDonations.reduce(function(total,val){
return total + val;
});
