...
let inventory = [
  ...
  console.log(`Car 33 is a *car year goes here* *car make goes here* *car model goes here*`);
   for(i = 0 ; i< inventory.length;i++){
       ...
       console.log("Car 33 is a "+inventory[i].car_year+" year and made by "+inventory[i].car_make+" with model "+inventory[i].car_model);
           break;
       }
   }
  ...
  console.log("challenge 2: Car make is "+inventory[lastCar].car_make+"and model "+inventory[lastCar].car_model);
...
for(yearCount=0;yearCount < carYears.length;yearCount++){
  console.log('car years['+carYears[yearCount]+']');
}
  
  // ==== Challenge 5 ====
  // The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
  let oldCars = [];
 for(z=0;z < carYears.length;z++){
     if(carYears[z].car_year < 2000){
     oldCars.push(carYears[z].car_year);
     }
}
for(j=0;j < oldCars.length;j++){
  console.log('Found cars year older that 2000 ['+oldCars[j]+']');
}
  ...
  var BMWAndAudi = [];
for(z=0;z < inventory.length;z++){
     if(inventory[z].car_make.toLowerCase()=="bmw" || inventory[z].car_make.toLowerCase()=="Audi"){
     BMWAndAudi.push(inventory[z].car_make);
     }
}
  for(j=0;j < BMWAndAudi.length;j++){
  console.log('Found cars  ['+JSON.stringify(BMWAndAudi[j])+']');
}